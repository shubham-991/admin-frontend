...
function App() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios.post('/search', { email: emailValue, password: passwordValue, keyword: keywordValue });
...
const handleDownloadAttachment = async (message_id, fileName) => {
  try {
    ...
    const response = await axios.get(`/download_attachment?message_id=${message_id}&file_name=${fileName}&username=${email}&password=${password}`, {
      responseType: 'blob'
    });
    ...
    try {
      const attachmentPromises = fromEmailsWithAttachments.map(async result => {
        const attachmentResponses = await Promise.all(result.attachments.map(async attachment => {
          const response = await axios.get(`/download_attachment?message_id=${result.id}&file_name=${attachment.filename}&username=${email}&password=${password}`, {
            responseType: 'blob'
          });
